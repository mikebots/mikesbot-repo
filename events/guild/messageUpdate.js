    ...
    Object.assign(message, newMessage)
       ...
       // if(message.content.startsWith(`<@${bot.user.id}> `)) prefix = `<@${bot.user.id}> `;
        ...
        if(command){
            ...
            checkEmbed.setTitle(`${command.name} has been disabled`)
            checkEmbed.setColor("RED");
            checkEmbed.setDescription(`${command.name} has been disabled on this server. Tell a server admin to re enable it.`)
            ...
            if(command.timeout){
                if(Timeout.has(`${message.author.id}${command.name}`)){
                        ...
                        embed.setTitle(`${command.name} is under cooldown!`)
                        embed.setColor("RED")
